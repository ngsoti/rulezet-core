const BadRuleMultiSource = {
    props: {
        modelValue: { type: Array, default: () => [] },
        placeholder: { type: String, default: 'Filter by sources...' },
        apiEndpoint: { type: String, default: '/rule/get_bad_rules_licenses_usage' },
    ...
    setup(props, { emit }) {
        ...
        const filteredList = Vue.computed(() => {
            if (!searchCtx.value) return list_sources.value;
        ...
        // MODIFICATION ICI : On retourne le nom tel quel sans transformation
        const getSourceDomain = (name) => {
            return name; 
        };
        ...
        const getSourceColor = (name) => {
            const n = name.toLowerCase();
            if (n.includes('github')) return 'bg-dark text-white';
            if (n.includes('gitlab')) return 'bg-warning text-dark';
            if (n.includes('bitbucket')) return 'bg-primary text-white';
        ...
        <div class="form-control d-flex flex-wrap gap-2 align-items-center p-2 shadow-sm "
            ...
            <span v-for="name in selectedNames" :key="name"
                ...
                <div class="px-2 py-1 d-flex align-items-center">
                    <span class="fw-bold me-2">[[ getSourceDomain(name) ]]</span>
        ...
        <div class="dropdown-menu shadow-lg border-0 w-100 p-3 mt-2 animate__animated animate__fadeIn"
            ...
            <div class="custom-tag-scroll pe-2" style="max-height: 400px; overflow-y: auto;">
                ...
                <div v-else class="d-flex flex-column gap-2">
                    <div v-for="source in filteredList" :key="source.name"
                         @click="toggleSource(source.name)"
                         class="p-2 rounded border d-flex align-items-center justify-content-between tag-item-hover"
                         :class="{'border-info bg-info-subtle': selectedNames.includes(source.name)}"
                         style="cursor:pointer;">
                        <div class="d-flex align-items-center flex-grow-1">
                            <i :class="['me-2', getSourceIcon(source.name)]" style="font-size: 0.85rem;"></i>
                            <span class="small fw-bold" style="color: var(--text-color);">[[ getSourceDomain(source.name) ]]</span>
                        </div>
                        <span class="badge rounded-pill bg-light border" style="font-size: 0.65rem; color: var(--text-color);">[[ source.count ]]</span>
                    </div>
                </div>
            </div>
        </div>
...
export default BadRuleMultiSource;